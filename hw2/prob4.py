...
# @Last Modified time: 2018-02-10 22:27:39
...
class pb3(hw2.hw2):
    ...
    def qa(self):
        ...
        for lb, sigma in zip(range(3), [0.3, 0.5, 1.0]):
            ...
            self.fig.savefig("figQ4a{:03d}_{:.1f}.png".format(
    ...
    def qb(self):
        ...
        self.ax.plot(xu, mstar, '-o', lw=8, label="mean", color=self.colorlist[5])
        self.ax.plot(xd, yd, '*', label="data", markersize=10, color='k')
        self.add_x_labels(cycle(["x"]), self.ax)
        self.add_y_labels(cycle(["y(x)"]), self.ax)
        self.add_legends(self.ax)
        self.set_tick_size(self.ax)
        self.fig.savefig("figQ4b{:03d}.png".format(lb), **self.figsave)
